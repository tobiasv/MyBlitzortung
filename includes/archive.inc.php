...
function bo_show_archive_map()
{
	...
	if ($cfg['archive'])
	{
		...
		if (!$strikes_available || $time < floor($start_time/3600/24)*3600*24 || $time > $end_time + 3600 * 24)
		{
			...
			$img_file = bo_bofile_url().'?map='.$map.'&blank&blank_background&'.BO_LANG_ARGUMENT.'='._BL().'';
		...
		else if ($ani)
		{
			...
			if ($ani_cfg['transparent'] === false)
			{
				...
				$bo_file_url = bo_bofile_url().'?map='.$map.'&'.BO_LANG_ARGUMENT.'='._BL().'&date=';
			}
			else
			{
				$img_file = bo_bofile_url().'?map='.$map.'&blank&'.BO_LANG_ARGUMENT.'='._BL().'';
				$bo_file_url = bo_bofile_url().'?map='.$map.'&transparent&'.BO_LANG_ARGUMENT.'='._BL().'&date=';
			}
		...
		else
		{
			...
			$img_file = bo_bofile_url().'?map='.$map.'&date='.$date_arg.'&'.BO_LANG_ARGUMENT.'='._BL();
...
function bo_show_archive_search()
{
	...
	if ($getit)
	{
		...
		while($row = $res->fetch_assoc())
		{
			...
			{
				$alt = _BL('Signals');
				$description .= '<img src=\''.bo_bofile_url().'?bo_graph='.$row['raw_id'].'&'.BO_LANG_ARGUMENT.'='._BL().'\' style=\'width:'.BO_GRAPH_RAW_W.'px;height:'.BO_GRAPH_RAW_H.'px\' class=\'bo_archiv_map_signal\' alt=\''.htmlspecialchars($alt).'\'>';
			}
...
function bo_show_archive_table($show_strike_list = false, $lat = null, $lon = null, $fuzzy = null)
{
	...
	else if ($strike_id && $strikes_before)
	{
		...
		$img_file = bo_bofile_url().'?map='.$map.'&blank&'.BO_LANG_ARGUMENT.'='._BL().'';
		$bo_file_url = bo_bofile_url().'?map='.$map.'&transparent&'.BO_LANG_ARGUMENT.'='._BL().($hyps ? '&hyps' : '').'&strike_id=';
	...
	{
		$last_signal_received_time = 0;
		if (!isset($_GET['bo_datetime_start']) && !$date && $station_id > 0 && $station_id == bo_station_id())
		{
			$sql = "SELECT MAX(time) time FROM ".BO_DB_PREF."strikes 
					WHERE raw_id > 0 AND time > NOW() - INTERVAL $hours_back HOUR";
			$row = BoDb::query($sql)->fetch_assoc();
			$last_signal_received_time = strtotime($row['time'].' UTC');
		}
		
		//default: display strikes older than last signal for the first call
		if ($last_signal_received_time && time() - $last_signal_received_time < 1800)
		{
			$time_end = $last_signal_received_time;
			$time_start = $last_signal_received_time - 3600 * $hours_back;
			$date = date('Y-m-d H:i', $time_start);
			$_GET['bo_datetime_start'] = $date; //workaround for page nav.
		}
		else
		{
			$time_end = time();
	...
	while($row = $res->fetch_assoc())
	{
		...
		if ( $perm && count($participated_stations) && ($strike_id || ($row['strike_id'] && $show_details)) )
		{
			...
			$img_dim = bo_archive_get_dim_html($map);
			$img_file = bo_bofile_url().'?map='.$map.'&strike_id='.$row['strike_id'].'&hyps&'.BO_LANG_ARGUMENT.'='._BL();
